...
public:
    ...
    fast_u8 read8(fast_u16 addr, memory_access_kind kind) {
        if(!use_custom_read_handler)
            return (*this)[addr];
        return on_read(addr, kind, ticks);
    }
    ...
    void write8(fast_u16 addr, fast_u8 value, memory_access_kind kind) {
        ...
        on_write(addr, value, kind, ticks);
    }

    fast_u16 read16(fast_u16 addr, memory_access_kind kind) {
        fast_u8 lo = read8(addr, kind, ticks);
        fast_u8 hi = read8(inc16(addr), kind, ticks);
    ...
    void write16(fast_u16 addr, fast_u16 value, memory_access_kind kind) {
        ...
        write8(addr, lo, kind, ticks);

        fast_u8 hi = static_cast<fast_u8>((value >> 8) & mask8);
        write8(inc16(addr), hi, kind, ticks);
    }

    void clear();

protected:
    virtual fast_u8 on_read(fast_u16 addr, access_kind kind);

    virtual void on_write(fast_u16 addr, fast_u8 value, access_kind kind);
